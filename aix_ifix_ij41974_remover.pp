#
#  2023/04/17 - cp - Building a remover for this fix because IJ44354 actually
#		includes it and refuses to install while this one is present.
#
#-------------------------------------------------------------------------------
#
#  From Advisory.asc:
...
#-------------------------------------------------------------------------------
#
class aix_ifix_ij41974_remover {
    ...
    if ($::facts['osfamily'] == 'AIX') {
        ...
        if (($ifixSuffix != 'unknown') and ($ifixBuildDate != 'unknown')) {
            ...
            if ($ifixFullName in $::facts['aix_ifix']['hash'].keys) {
                ...
                $ifixStagingSource = "puppet:///modules/aix_ifix_ij41974_remover/${ifixName}${ifixSuffix}.${ifixBuildDate}.epkg.Z"
